...
// Esta nueva función centralizará la obtención de datos desde nuestra API "puente".
const API_BASE_URL = 'https://observauto-data-bridge.vercel.app/api'; // URL de ejemplo, la reemplazaremos por la real
...
async function fetchData(startDate, endDate, compareStartDate = null, compareEndDate = null) {
    ...
    try {
        ...
        throw error; // Propagamos el error para que sea manejado más arriba
    }
}
...
// La función `render` ahora es asíncrona (async) para poder esperar
// la respuesta de nuestra nueva función `fetchData`.
async function render() {
    ...
    // Oculta todas las vistas y actualiza la navegación
...
function renderConsolidado() {
    ...
    // Esta función se adaptará para usar los datos consolidados de STATE.data
    view.innerHTML = `
        <div class="card card-dark">
            ...
            <ul>
                <li>El tráfico web muestra una tendencia positiva, con un aumento del <strong>${STATE.data.website_bench.delta.sessions_d.toFixed(1)}%</strong> en sesiones.</li>
                <li>Instagram sigue siendo la red con mayor alcance, llegando a <strong>${fmt(STATE.data.instagram.reach.periodA)}</strong> personas.</li>
                <li>El contenido en YouTube generó <strong>${fmt(STATE.data.youtube.views.periodA)}</strong> nuevas visualizaciones.</li>
            </ul>
        </div>
...
function renderWebsite() {
    ...
    // Crear contenedor para el gráfico y luego renderizarlo
...
function load() {
    ...
    createNav();
    bindNav();
    handleDateChange(); // Esto iniciará el primer render y la primera llamada a la API
}
